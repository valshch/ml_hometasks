...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        dists = np.sqrt(np.power(self.X_train, 2).sum(axis=1) + np.power(X, 2).sum(axis=1)[:, np.newaxis] - 2. * X@self.X_train.transpose())
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            label, number = np.unique(closest_y, return_counts = True)
            y_pred[i] = label[number == number.max()].min()
